...
import { anyContextWithApp, anyData, anyEmptyContext } from "./helpers/testConstants";
...
describe('Testing "helloWorld"', () => {
  it("test helloWorld if function works", async () => {
    const result: Message = await testEnv.wrap(helloWorld)(anyData, anyContextWithApp);
    expect(result.code).toBe(200);
    expect(result.message).toBe("Hello from Firebase!");
  });

  it("test helloWorld throws error because no context was given", async () => {
    const error = await getError(async () => await testEnv.wrap(helloWorld)(anyData, anyEmptyContext));
